...
CREATE TABLE `boostack_user` (
  ...
  `passkey_prompt_disabled` tinyint(1) NOT NULL DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;
...
ALTER TABLE `boostack_log`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_boostack_log_session_datetime` (`session_id`,`datetime`),
...
ALTER TABLE `boostack_user_cache`
  ADD UNIQUE KEY `uniq_boostack_user_cache_user_key` (`id_user`,`key`),
  ADD KEY `id_user` (`id_user`);
...
ALTER TABLE `boostack_user_passkey`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uk_user_passkey_credential_id` (`credential_id`),
...
ALTER TABLE `boostack_user_role`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `id_user` (`id_user`,`id_role`) USING BTREE,
  ADD UNIQUE KEY `ux_boostack_user_role_user_role` (`id_user`,`id_role`),
