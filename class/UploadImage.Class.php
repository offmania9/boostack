...
class UploadImage{
	...
	private $image_types = array("image/gif","image/jpeg","image/pjpeg","image/bmp","image/png");
    ...
    public function constraints($file) {
		...
		if(strlen($file["name"]) >= $MAX_UPLOAD_NAMEFILE_LENGTH){ ## FILE NAME TOO LONG
  ...
  function resizeToHeight($height, $filter=NULL) {
   ...
   function resize($width,$height, $filter=NULL) {
	  ...
	  switch($filter){
		  	case "la":{imagefilter($new_image, IMG_FILTER_CONTRAST, -40);break;}
			case "ny":{imagefilter($new_image, IMG_FILTER_GRAYSCALE);break;}
			case "sd":{imagefilter($new_image, IMG_FILTER_COLORIZE, 0, 0, 100);break;}
			case "sf":{imagefilter($new_image, IMG_FILTER_COLORIZE, 0, 100, 0);break;}
	  }
	  	if($this->type == "image/jpeg" || $this->type == "image/pjpeg")
