    ...
    public function processAPI()
    {
        try {
            if (!Utils::checkAcceptedTimeFromLastRequest(Auth::getLastTry())) {
                throw new Exception_APITooManyRequests("Too many requests. Please wait a few seconds.");
            }

            $methodBindings = [];
            $subclasses = [];
            $dir = ROOTPATH . "../classes/Rest";
            $declaredClasses = [];
            ...
            if (isset($methodBindings[$this->endpoint])) {
                ...
                throw new Exception_APINotFound("No Endpoint: " . $this->endpoint . ". The resource you requested doesn't exist. For more info, please refer to the documentation.");
            }
